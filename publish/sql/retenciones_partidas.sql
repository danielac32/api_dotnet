SELECT DISTINCT p.anho presupuesto,
       ...
       r.RETE_ID||'-'||rt.DENOMINACION DENOMINACION,
...
WHERE p.anho = pi.anho
  ...
  AND P.ORGA_ID = D.ORGA_ID
  AND P.TIPG_ID = D.TIPG_ID
  AND P.PAGO_ID = D.PAGO_ID
  AND P.ANHO 	= D.ANHO
  AND D.PLUC_ID = PU.PLUC_ID
  AND D.FFIN_ID = F.FFIN_ID
